    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateUser([FromBody] CreateUserRequest request)
        {
            ...
            var user = new User
            {
                ...
                Status = request.Status?.ToLowerInvariant() == "active",
                RoleId = role.RoleId,
                LastLogin = null
            };
        ...
        public async Task<IActionResult> ChangeRole(string id, [FromBody] ChangeRoleRequest request)
        {
            ...
            var (success, userDto, errorMessage) = await UpdateUserInternal(id, null, null, null, null, request.Role, null);
            if (!success)
            {
                if (errorMessage.Contains("not found")) return NotFound(new { message = errorMessage });
                return BadRequest(new { message = errorMessage });
        ...
        private async Task<(bool Success, UserDto? UserDto, string ErrorMessage)> UpdateUserInternal(string id, string? nom, string? prenom, string? email, string? password, string? role, string? status)
        {
            ...
            if (nom != null) user.Nom = nom.Trim();
            if (prenom != null) user.Prenom = prenom.Trim();
            if (email != null)
            {
            ...
            if (!string.IsNullOrEmpty(password))
                user.MotDePasse = BCrypt.Net.BCrypt.HashPassword(password);
            if (status != null)
            ...
            var dto = new UserDto
            {
                ...
                Email = updated.Email,
                Role = roleEntity2?.Nom ?? string.Empty,
